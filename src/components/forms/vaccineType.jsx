...
import { Stack, Heading, Box, Button, HStack, Text, Flex } from '@chakra-ui/react';
...
const VaccineType = ({ setApprovedVaccinated }) => {
  ...
  const updateVaccines = (id, change) => {
    setVaccines(
      vaccines.map((v) => (v.id == id ? { ...v, count: rangeValue(v.count + change, 5, 0) } : v))
    );
  };

  const vaccineScore = () => {
    return vaccines.reduce((s, v) => s + v.score * v.count, 0);
  };

  const vaccineAmount = () => {
    return vaccines.reduce((s, v) => s + v.count, 0);
  };
